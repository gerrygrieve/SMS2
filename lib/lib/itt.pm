...
$xml_element = "it_task";
$sub_element = "task";
my %elements;					#A hash to hold attributes
...
{
    my $class = shift;
    my $self = { _permitted => \%elements, %elements};
    bless $self, $class;
...
{
   my $self = shift;
 #  print "Survey::AUTOLOAD: self--> $self\n"; 
   my $type = ref($self) || die "<<$self>> is not an object\n";
   my $name = $AUTOLOAD;
    $name =~ s/.*://;
    return if $name eq 'DESTROY';
  #  print "Survey::AUTOLOAD: name--> $name\n"; 
...
#####    return Survey object reference...
 ...
 #   print "<br />Survey::rd_file:: $xml_file opened OK <br /> \n";
  
   
    m[<$xml_element>(.*?)</$xml_element>]msg;
    my $talk = $1;
 #   print "talk is $talk \n";
# toss the modifiction stuff      
    $talk =~ s[<modified>.*</modified>][]msg;
    my $t = new  itt;
  # print "<br />Survey::rd_file:: created new Object<br /> \n";
	...
	my $value = $2;
        $value =~ s/\s*$//;
 #       print "Survey::rd_file:: tag ${field} has value $value<br /> \n";
